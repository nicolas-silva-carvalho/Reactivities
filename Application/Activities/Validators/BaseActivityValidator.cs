...
public class BaseActivityValidator<T, TDto> : AbstractValidator<T> where TDto : BaseActivityDto
{
    public BaseActivityValidator(Func<T, TDto> selector)
    {
        RuleFor(x => selector(x).Title)
            ...
            .MaximumLength(100).WithMessage("Title must not exced 100 characters");
        ...
        RuleFor(x => selector(x).Latitute)
            .NotEmpty().WithMessage("Latitute is required")
            .InclusiveBetween(-90, 90).WithMessage("Latitute must be between -90 and 90");
